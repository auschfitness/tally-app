...
- Próximo na migração: **Events/Eventos** (/events). Banco pronto e verificado — handoff em `docs/handoffs/events-supabase.md`. ⚠️ 2 alertas: (a) `event_registrations` SEM unique — dedupe no app, não upsert; (b) dois check-ins distintos (`event_registrations.checked_in` vs `attendance_records` context_type='event') — checar o legado. Inscrição pública+pagamento ADIADA e exige RLS anon que EU crio depois. Depois: Calendar (só agrega), Study→Tracks, Care→Inbox→Home, Settings.
