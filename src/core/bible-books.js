// Lista canônica dos 66 livros (Step 5 · Fase 3). O `code` é USFM 3 letras
// (GEN, JHN, 1CO) — o MESMO identificador que a Free Use Bible API (helloao) usa
// para buscar capítulos, e o código gravado em sermon_scriptures.book. `order` dá a
// sequência 1-66 do Scripture Map; `pt`/`en` os nomes de exibição; `abbr` as formas
// que o parser aceita (nomes + abreviações PT/EN). O parser normaliza tudo isto.
...
export var BOOKS = RAW.map(function (r) {
  return { code: r[0], order: r[1], pt: r[2], en: r[3], abbr: r[4] };
});

// Mapa nome/abreviação normalizado → código canônico (para o parser).
export var NAME_TO_CODE = (function () {
  var m = {};
  BOOKS.forEach(function (b) {
    [b.pt, b.en].concat(b.abbr).forEach(function (form) { m[normToken(form)] = b.code; });
  });
  return m;
})();

var CODE_TO_BOOK = (function () { var m = {}; BOOKS.forEach(function (b) { m[b.code] = b; }); return m; })();
export function bookByCode(code) { return CODE_TO_BOOK[code] || null; }
// Nome de exibição do livro (PT por padrão — o app é PT).
export function bookName(code) { var b = CODE_TO_BOOK[code]; return b ? b.pt : code; }
